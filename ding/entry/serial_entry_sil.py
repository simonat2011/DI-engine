...
def serial_pipeline_sil(
    ...
    if env_setting is None:
        env_fn, collector_env_cfg, evaluator_env_cfg = get_vec_env_setting(
            cfg.env)
    else:
    ...
    learner = BaseLearner(cfg.policy.learn.learner,
                          policy.learn_mode, tb_logger, exp_name=cfg.exp_name)
    ...
    if cfg.policy.get('random_collect_size', 0) > 0:
        ...
        new_data = collector.collect(
            n_sample=cfg.policy.random_collect_size, policy_kwargs=collect_kwargs)
    ...
    for _ in range(max_iterations):
        ...
        for i in range(cfg.policy.learn.update_per_collect):
            ...
            train_data_base_policy = replay_buffer.sample(
                learner.policy.get_attribute('batch_size'), learner.train_iter, sample_range=slice(old_ptr-new_ptr))
            ...
            if train_data_base_policy is None or train_data_sil is None:
                ...
                break
            learner.train({'base_policy': train_data_base_policy,
                          'sil': train_data_sil}, collector.envstep)
